...
static int connect_to(unsigned short port)
{
    ...
    assert_int_eq(connect(fd, (struct sockaddr *)&addr, sizeof(addr)), 0);
...
static unsigned short start_server(int *passive_out)
{
    ...
    assert_int_eq(selector_init(&conf), SELECTOR_SUCCESS);
    test_selector = selector_new(64);
    assert_ptr_nonnull(test_selector);

    int passive = server_setup_passive("127.0.0.1", 0);
    assert_int_ge(passive, 0);
    struct sockaddr_in bound;
    socklen_t          bound_len = sizeof(bound);
    assert_int_eq(getsockname(passive, (struct sockaddr *)&bound, &bound_len), 0);

    assert_int_ge(selector_fd_set_nio(passive), 0);
                                                    ...
                                                    int client_fd)
{
    ...
    assert_int_eq(selector_register(s, client_fd, &socks5_handler, OP_NOOP, c),
                     SELECTOR_SUCCESS);
    conn_list_push(c);
    active_connections++;
    return c;
...
static struct origin_ctx start_origin(void)
{
    ...
    assert_int_eq(listen(fd, 1), 0);
    socklen_t len = sizeof(addr);
    assert_int_eq(getsockname(fd, (struct sockaddr *)&addr, &len), 0);
...
static void negotiate_noauth(int client)
{
    ...
    assert_int_eq(write(client, neg, sizeof(neg)), (ssize_t)sizeof(neg));
    uint8_t reply[2];
    read_exactly(client, reply, sizeof(reply));
    assert_uint_eq(0x05, reply[0]);
    assert_uint_eq(0x00, reply[1]);
}
...
static void send_connect_ipv4(int client, unsigned short port)
{
    ...
    assert_int_eq(write(client, req, sizeof(req)), (ssize_t)sizeof(req));
}
...
static void send_connect_ipv6_loopback(int client, unsigned short port)
{
    ...
    assert_int_eq(inet_pton(AF_INET6, "::1", &req[4]), 1);
...
static struct socks5_reply_info read_socks5_reply_info(int client)
{
    ...
    assert_uint_eq(0x05, header[0]);
    assert_uint_eq(0x00, header[2]);
...
static void
test_socks5_negotiation_then_connect(void)
{
    socks5_set_users(&no_users);

    struct origin_ctx origin = start_origin();
    pthread_t origin_tid;
    assert_int_eq(pthread_create(&origin_tid, NULL, origin_echo_run, &origin), 0);
    ...
    test_stop = 0;
    pthread_t loop;
    assert_int_eq(pthread_create(&loop, NULL, run_selector, NULL), 0);
    ...
    assert_uint_eq(SOCKS5_REP_SUCCESS, reply.rep);
    assert_uint_eq(SOCKS5_ATYP_IPV4, reply.atyp);
    uint8_t loopback[4];
    assert_int_eq(inet_pton(AF_INET, "127.0.0.1", loopback), 1);
    assert_int_eq(memcmp(loopback, reply.addr, sizeof(loopback)), 0);
    assert_uint_ne(0, reply.port);
 ...
 * reply. */
static void
test_socks5_bad_version_closes(void)
{
    int            passive;
    unsigned short port = start_server(&passive);
    ...
    assert_int_eq(write(client, bad, sizeof(bad)), (ssize_t)sizeof(bad));

    uint8_t scratch[8];
    ssize_t r = read(client, scratch, sizeof(scratch));
    assert_int_eq(r, 0); /* closed without a reply */
...
static void
test_socks5_userpass_valid_advances(void)
{
    ...
    assert_uint_eq(0x05, neg_reply[0]);
    assert_uint_eq(0x02, neg_reply[1]);

    /* RFC 1929 auth: VER=1, ULEN=5 "alice", PLEN=6 "secret" */
    uint8_t auth[] = { 0x01, 0x05, 'a','l','i','c','e', 0x06, 's','e','c','r','e','t' };
    assert_int_eq(write(client, auth, sizeof(auth)), (ssize_t)sizeof(auth));
    uint8_t auth_reply[2];
    read_exactly(client, auth_reply, sizeof(auth_reply));
    assert_uint_eq(0x01, auth_reply[0]);
    assert_uint_eq(0x00, auth_reply[1]); /* success */

    send_connect_ipv4(client, origin.port);
    assert_uint_eq(SOCKS5_REP_SUCCESS, read_socks5_reply(client));
...
static void
test_socks5_userpass_invalid_rejected(void)
{
    static const struct socks5args args = { .users = { { .name = "alice", .pass = "secret" } } };
    socks5_set_users(&args);
    ...
    read_exactly(client, auth_reply, sizeof(auth_reply));
    assert_uint_eq(0x01, auth_reply[0]);
    assert_uint_eq(0x01, auth_reply[1]); /* failure */

    /* server must close after rejecting -> EOF */
    uint8_t scratch[8];
    assert_int_eq(read(client, scratch, sizeof(scratch)), 0);
...
static void
test_socks5_pipelined_request(void)
{
    socks5_set_users(&no_users);
    ...
    assert_int_eq(write(client, pipelined, sizeof(pipelined)),
...
static void
test_socks5_no_acceptable_method_closes(void)
{
    socks5_set_users(&no_users);
    ...
    read_exactly(client, reply, sizeof(reply));
    assert_uint_eq(0x05, reply[0]);
    assert_uint_eq(0xFF, reply[1]);

    uint8_t scratch[8];
    assert_int_eq(read(client, scratch, sizeof(scratch)), 0); /* closed */
...
static void
test_socks5_auth_required_rejects_noauth(void)
{
    static const struct socks5args args = { .users = { { .name = "alice", .pass = "secret" } } };
    socks5_set_users(&args);
 ...
 * auth phases reply. */
static void
test_socks5_request_error_replies(void)
{
    socks5_set_users(&no_users);
    ...
    assert_int_eq(write(client, bad_req, sizeof(bad_req)), (ssize_t)sizeof(bad_req));

    uint8_t rep[10];
    read_exactly(client, rep, sizeof(rep));
    assert_uint_eq(0x05, rep[0]);
    assert_uint_eq(SOCKS5_REP_GENERAL_FAILURE, rep[1]);
    assert_uint_eq(0x00, rep[2]);          /* RSV */
    assert_uint_eq(SOCKS5_ATYP_IPV4, rep[3]);
...
static void
test_socks5_non_connect_rejected(void)
{
    socks5_set_users(&no_users);
    ...
    read_exactly(client, rep, sizeof(rep));
    assert_uint_eq(0x05, rep[0]);
    assert_uint_eq(SOCKS5_REP_CMD_NOT_SUPPORTED, rep[1]);
...
test_socks5_relay_echo(void)
{
    ...
    assert_int_eq(write(client, msg, msg_len), (ssize_t)msg_len);

    uint8_t echoed[64];
    read_exactly(client, echoed, msg_len);
    assert_int_eq(memcmp(msg, echoed, msg_len), 0);
...
static void
test_socks5_ipv6_connect_reply_uses_ipv6_atyp(void)
{
    socks5_set_users(&no_users);
    ...
    struct socks5_reply_info reply = read_socks5_reply_info(client);
    assert_uint_eq(SOCKS5_REP_SUCCESS, reply.rep);
    assert_uint_eq(SOCKS5_ATYP_IPV6, reply.atyp);
    struct in6_addr loopback6;
    assert_int_eq(inet_pton(AF_INET6, "::1", &loopback6), 1);
    assert_int_eq(memcmp(&loopback6, reply.addr, sizeof(loopback6)), 0);
    assert_uint_ne(0, reply.port);
...
static void
test_socks5_relay_propagates_client_half_close(void)
{
    socks5_set_users(&no_users);

    struct half_close_origin_ctx origin = start_half_close_origin();
    pthread_t origin_tid;
    assert_int_eq(pthread_create(&origin_tid, NULL, origin_reply_after_eof_run, &origin), 0);
    ...
    size_t msg_len = strlen(msg);
    assert_int_eq(write(client, msg, msg_len), (ssize_t)msg_len);
    assert_int_eq(shutdown(client, SHUT_WR), 0);

    static const char expected_reply[] = "origin saw eof";
    uint8_t reply[sizeof(expected_reply) - 1];
    read_exactly(client, reply, sizeof(reply));
    assert_int_eq(memcmp(expected_reply, reply, sizeof(reply)), 0);

    uint8_t scratch[8];
    assert_int_eq(read(client, scratch, sizeof(scratch)), 0);

    pthread_join(origin_tid, NULL);
    assert_int_eq(origin.saw_eof, 1);
    assert_uint_eq(msg_len, origin.received_len);
    assert_int_eq(memcmp(msg, origin.received, msg_len), 0);
...
static void
test_socks5_connect_refused(void)
{
    socks5_set_users(&no_users);
    ...
    assert_uint_eq(SOCKS5_REP_CONNECTION_REFUSED, rep_code);
...
static void
test_socks5_fqdn_resolution_failure_is_general_failure(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
    ...
    assert_uint_eq(REQ_WRITE, request_resolve_done(&key));

    size_t pending;
    uint8_t *reply = buffer_read_ptr(&c->write_buffer, &pending);
    assert_uint_eq(10, pending);
    assert_uint_eq(SOCKS5_REP_GENERAL_FAILURE, reply[1]);

    selector_unregister_fd(s, fds[0]);
    assert_uint_eq(0, socks5_active_connections());
    close(fds[1]);
    selector_destroy(s);
    selector_close();
}
 ...
 * path. */
static void
test_socks5_fqdn_connect_and_relay(void)
{
    socks5_set_users(&no_users);
    ...
    assert_int_eq(write(client, req, req_len), (ssize_t)req_len);
...
static void
test_socks5_reap_idle_connection(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
    ...
    assert_uint_eq(socks5_active_connections(), 0);
 ...
 * must remain alive. */
static void
test_socks5_reap_keeps_relay_before_relay_timeout(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
    ...
    int client_fds[2];
    int origin_fds[2];
    assert_int_eq(socketpair(AF_UNIX, SOCK_STREAM, 0, client_fds), 0);
    assert_int_eq(socketpair(AF_UNIX, SOCK_STREAM, 0, origin_fds), 0);

    struct socks5_conn *c = new_registered_test_conn(s, client_fds[0]);
    c->origin_fd = origin_fds[0];
    c->references++;
    assert_int_eq(selector_register(s, origin_fds[0], &socks5_handler, OP_NOOP, c),
                     SELECTOR_SUCCESS);
    ...
    reap_last_sweep = 0;
    socks5_reap_idle(s);
    assert_uint_eq(1, socks5_active_connections());
...
static void
test_socks5_reap_closes_idle_relay_after_relay_timeout(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
...
static void
test_socks5_reap_req_connecting_sends_failure_reply(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
    ...
    reap_last_sweep = 0;
    socks5_reap_idle(s);
    assert_uint_eq(REQ_WRITE, stm_state(&c->stm));
    assert_uint_eq(1, socks5_active_connections());
    ...
    assert_uint_eq(SOCKS5_REP_TTL_EXPIRED, reply[1]);
    assert_uint_eq(0, socks5_active_connections());
...
static void
test_socks5_reap_req_resolve_timeout_sends_failure_reply(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
...
static void
test_socks5_reap_req_resolve_timeout_releases_pending_job(void)
{
    socks5_resolver_pool_stop();
    ...
    socks5_reap_idle(s);
    assert_uint_eq(REQ_WRITE, stm_state(&c->stm));
    assert_uint_eq(0, resolver_jobs_in_system);
    assert_ptr_null(c->resolver_job);
    assert_int_eq(1, c->references);
...
static void
test_socks5_reap_completed_resolve_without_notification(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
...
static void
test_socks5_reap_completed_resolve_refreshes_activity(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
    ...
    socks5_reap_idle(s);
    assert_uint_eq(REQ_WRITE, stm_state(&c->stm));
    assert_int_ge(c->last_activity, before);
...
static void
test_socks5_reap_req_write_with_origin_fd(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
...
static void
test_socks5_relay_compacts_partial_drain_before_interest_update(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
    ...
    assert(!buffer_can_write(&c->read_buffer));

    assert_uint_eq(RELAY, relay_update(c));
    assert(buffer_can_write(&c->read_buffer));
    assert(c->client_interest & OP_READ);
...
static void
test_socks5_resolver_pool_rejects_when_capacity_is_exhausted(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
...
static void
test_socks5_rejects_domain_with_embedded_nul(void)
{
    socks5_set_users(&no_users);
    ...
    assert_int_eq(write(client, req, sizeof(req)), (ssize_t)sizeof(req));
    assert_uint_eq(SOCKS5_REP_GENERAL_FAILURE, read_socks5_reply(client));
...
static void
test_socks5_close_cancels_pending_resolver_reference(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
    ...
    assert_uint_eq(1, socks5_active_connections());
    assert_int_eq(selector_unregister_fd(s, fds[0]), SELECTOR_SUCCESS);
    assert_uint_eq(0, socks5_active_connections());
    assert_uint_eq(0, resolver_jobs_in_system); /* close cancelled and freed it */
...
static void
test_socks5_resolver_job_captures_notify_target(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
    ...
    assert_ptr_nonnull(job);
    assert_int_eq(fds[0], job->notify_fd);
    assert_ptr_eq(s, job->notify_selector);
    pthread_mutex_unlock(&resolver_mutex);
...
static void
test_socks5_relay_init_arming_failure_tears_down(void)
{
    struct selector_init conf = {
        .signal         = SIGALRM,
        .select_timeout = { .tv_sec = 0, .tv_nsec = 50000000 },
    ...
    assert_uint_eq(SOCKS5_REP_SUCCESS, reply[1]);
    assert_uint_eq(0, socks5_active_connections());

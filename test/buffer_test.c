...
test_buffer_misc(void)
{
    ...
    assert_int_eq(true,  buffer_can_write(b));
    assert_int_eq(false, buffer_can_read(b));

    size_t wbytes = 0, rbytes = 0;
    uint8_t *ptr = buffer_write_ptr(b, &wbytes);
    assert_uint_eq(6, wbytes);
    // escribo 4 bytes
    uint8_t first_write [] = {
        'H', 'O', 'L', 'A',
    };
    ...
    assert_uint_eq(2, wbytes);

    // tengo por leer
    buffer_read_ptr(b, &rbytes);
    assert_uint_eq(4, rbytes);

    // leo 3 del buffer
    assert_uint_eq('H', buffer_read(b));
    assert_uint_eq('O', buffer_read(b));
    assert_uint_eq('L', buffer_read(b));

    // queda 1 por leer
    buffer_read_ptr(b, &rbytes);
    assert_uint_eq(1, rbytes);
    ...
    assert_int_eq(false, buffer_can_write(b));
    buffer_write_ptr(b, &wbytes);
    assert_uint_eq(0, wbytes);

    // tiene que haber 2 + 1 para leer
    ptr = buffer_read_ptr(b, &rbytes);
    assert_uint_eq(3, rbytes);
    assert_ptr_ne(ptr, b->data);

    buffer_compact(b);
    assert_ptr_eq(b->data, buffer_read_ptr(b, &rbytes));
    assert_uint_eq(3, rbytes);
    assert_ptr_eq(b->data + 3, buffer_write_ptr(b, &wbytes));
    assert_uint_eq(3, wbytes);
    ...
    buffer_read_ptr(b, &rbytes);
    assert_uint_eq(0, rbytes);
    assert_ptr_eq(b->data, buffer_write_ptr(b, &wbytes));
    assert_uint_eq(6, wbytes);
    ...
    assert_uint_eq(0, rbytes);
    buffer_write_ptr(b, &wbytes);
    assert_uint_eq(N(direct_buff), wbytes);
